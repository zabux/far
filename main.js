...
console.log(color('F', 'green'), color('a', 'red'), color('r', 'yellow'), color('d', 'purple'), color('a', 'blue'),  color('n', 'green'), color('BOT', 'cyan'))
                      console.log(color('SUPPORT by htttps://youtube.com/FaxBotz', 'cyan'))
...
//GRUP UPDATE
mans.ev.on("messages.reaction", async (m) => {
console.log(m)
//if (!antireact.includes(m.id)) return
const ngrm = m.reaction.key.fromMe ? mans.decodeJid(mans.user.id) : mans.decodeJid(m.reaction.key.participant)
const botNumber = await mans.decodeJid(mans.user.id)
if (ngrm === botNumber) return
let mesg = await store.loadMessage(m.reaction.key.remoteJid, m.key.id, mans);
teks = `*── 「 DETECTED 」 ──*

@${ngrm.split("@")[0]} Telah Mengirim Reaction ${
m.key.participant ? `ke @${m.key.participant.split("@")[0]}` : ``
}\n*Emoji Reaction:* ${m.reaction.text}`
await mans.sendMessage(
m.reaction.key.remoteJid,
{
text: teks,
contextInfo:{mentionedJid: await mans.parseMention(teks), externalAdReply:{
title: "Fax bot Official",
...
if (!wlcm.includes(anu.id)) return
        try {
            ...
            let reSiz = async(buffer, ukur1, ukur2) => {
...
mans.groupRevokeInvite(m.chat)
                ...
                name = await mans.getName(num)
              let cug =  `https://api.dhamzxploit.my.id/api/canvas/welcome2?name=${name}&mem=${encodeURIComponent(behys)}&gcname=${encodeURIComponent(metadata.subject)}&picurl=${encodeURIComponent(ppuser)}&bgurl=https://i.ibb.co/gwT1HxL/308f40b82bfa.jpg?size=300&ext=jpg`
              let qqppp = [{
                    "buttonId": `#sewabot`,
                    "buttonText": {
                        "displayText": "Sewa"
                ...
                mans.sendMessage(anu.id, {
            caption: `Hai @${num.split("@")[0]} Selamat Datang Di ${metadata.subject}`,
            location: {
                jpegThumbnail: await reSiz(cug, 200,200,)
            },
            buttons: qqppp,
            footer: `Welcome Massage By ${namebot}`,
            mentions:[num]
        },
    )
         //mans.sendMessage(anu.id, { image: { url: cug }, contextInfo: { mentionedJid: [num] }, caption: `Hai @${num.split("@")[0]} Selamat Datang Di ${metadata.subject}` })
                } else if (anu.action == 'remove') {
                    nameee = await mans.getName(num)
                    let cugy = `https://api.dhamzxploit.my.id/api/canvas/goodbye2?name=${nameee}&mem=${encodeURIComponent(behys)}&gcname=${encodeURIComponent(metadata.subject)}&picurl=${encodeURIComponent(ppuser)}&bgurl=https://i.ibb.co/gwT1HxL/308f40b82bfa.jpg?size=626&ext=jpg`
                    let qqppp = [{
                    "buttonId": `#sewabot`,
                    "buttonText": {
                        "displayText": "Sewa"
                ...
                mans.sendMessage(anu.id, {
            caption: `Soyanara @${num.split("@")[0]} Jangan Lupa beliin Softex😋`,
            location: {
                jpegThumbnail: await reSiz(cugy, 200,200,)
            },
            buttons: qqppp,
            footer: `Welcome Massage By ${namebot}`,
            mentions:[num]
